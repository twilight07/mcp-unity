...
export function createUpdateGameObjectComponentTool(mcpUnity, logger) {
    ...
    return {
        ...
        handler: async (params) => {
            ...
            if (params.instanceId === undefined || params.instanceId === null) {
                throw new McpUnityError(ErrorType.VALIDATION, "Required parameter 'instanceId' must be provided");
            }
            ...
            if (!params.componentData || Object.keys(params.componentData).length === 0) {
                throw new McpUnityError(ErrorType.VALIDATION, "Required parameter 'componentData' must be provided and not empty");
            }
            ...
            if (!response.success) {
                throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to update component on GameObject with ID ${params.instanceId}`);
            }
            return {
                success: true,
