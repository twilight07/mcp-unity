...
export function createPackageManagerTool(mcpUnity: McpUnity, logger: Logger): ToolDefinition {
    ...
    handler: async (params): Promise<CallToolResult> => {
      ...
      if (!response.success) {
        throw new McpUnityError(
          ...
          response.message || `Failed to manage package with method: ${methodSource}`
        );
      }
